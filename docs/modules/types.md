[lichen](../README.md) / [Exports](../modules.md) / types

# Module: types

## Table of contents

### Interfaces

- [ColorScaleOptions](../interfaces/types.ColorScaleOptions.md)
- [Coordinates](../interfaces/types.Coordinates.md)
- [CrosshairOptions](../interfaces/types.CrosshairOptions.md)
- [DataFromPos](../interfaces/types.DataFromPos.md)
- [DataUtilsComputedData](../interfaces/types.DataUtilsComputedData.md)
- [DataUtilsComputedSerieData](../interfaces/types.DataUtilsComputedSerieData.md)
- [EvenUtilsEventHandlerMap](../interfaces/types.EvenUtilsEventHandlerMap.md)
- [EventChannelSubscription](../interfaces/types.EventChannelSubscription.md)
- [GlobalAxisOptions](../interfaces/types.GlobalAxisOptions.md)
- [HeaderOptions](../interfaces/types.HeaderOptions.md)
- [Heatmap2dOptions](../interfaces/types.Heatmap2dOptions.md)
- [Heatmap3dOptions](../interfaces/types.Heatmap3dOptions.md)
- [HooksOptions](../interfaces/types.HooksOptions.md)
- [LegendItem](../interfaces/types.LegendItem.md)
- [LegendOptions](../interfaces/types.LegendOptions.md)
- [LichenOptions](../interfaces/types.LichenOptions.md)
- [LineOptions](../interfaces/types.LineOptions.md)
- [ScatterOptions](../interfaces/types.ScatterOptions.md)
- [ScatterPoint](../interfaces/types.ScatterPoint.md)
- [SequenceOptions](../interfaces/types.SequenceOptions.md)
- [StackedDataOptions](../interfaces/types.StackedDataOptions.md)
- [StackedOptions](../interfaces/types.StackedOptions.md)
- [TooltipHandlerResponse](../interfaces/types.TooltipHandlerResponse.md)
- [TooltipOptions](../interfaces/types.TooltipOptions.md)
- [VLine](../interfaces/types.VLine.md)
- [XAxisOptions](../interfaces/types.XAxisOptions.md)
- [YAxisOptions](../interfaces/types.YAxisOptions.md)

### Type Aliases

- [ColorScaleObject](types.md#colorscaleobject)

## Type Aliases

### ColorScaleObject

Ƭ **ColorScaleObject**: [`number`, [`number`, `number`, `number`], string?][]

#### Defined in

types.ts:38
